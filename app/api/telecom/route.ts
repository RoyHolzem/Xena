...
async function scanAllRecords(view: TelecomView) {
  ...
  do {
    ...
    records.push(...(response.Items || []).map((item) => normalize(view, item as RawItem)));
    exclusiveStartKey = response.LastEvaluatedKey;
  } while (exclusiveStartKey);

  return sortRecords(view, records);
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    let items = await scanAllRecords(view);
...
export async function POST(request: NextRequest) {
  ...
  const attempts = requestedRecordId ? 1 : 5;
