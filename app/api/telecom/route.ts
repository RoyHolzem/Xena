...
function getRequestedRecordId(body: Record<string, unknown>): string | null {
...
function buildCreateItem(view: TelecomView, body: Record<string, unknown>, recordId: string, now: string): Record<string, unknown> {
...
export async function POST(request: NextRequest) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    const item = buildCreateItem(view, body, recordId, now);
  ...
  return NextResponse.json({ ok: false, error: 'Could not generate a unique record ID. Please retry.' }, { status: 409 });
}
