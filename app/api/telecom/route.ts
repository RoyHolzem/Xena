...
export async function POST(request: NextRequest) {
  ...
  if (body.recordId !== undefined && (typeof body.recordId !== 'string' || !body.recordId.trim())) {
    return NextResponse.json({ ok: false, error: 'recordId must be a non-empty string' }, { status: 400 });
  }
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    const item: Record<string, unknown> = { recordId, createdAt: now, updatedAt: now, startTime: now, ...fields };
    ...
    try {
      ...
      if ((error as { name?: string }).name === 'ConditionalCheckFailedException') {
        ...
        return NextResponse.json(
          { ok: false, error: requestedRecordId ? `Record ${recordId} already exists` : 'Failed to generate a unique recordId' },
          { status: 409 },
        );
      }
  ...
  return NextResponse.json({ ok: false, error: 'Failed to generate a unique recordId' }, { status: 409 });
}
