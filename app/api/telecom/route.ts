...
async function scanAllRecords(view: TelecomView): Promise<RawItem[]> {
  const items: RawItem[] = [];
...
export async function GET(request: NextRequest) {
  ...
  try {
    let items = sortRecords(view, (await scanAllRecords(view)).map((item) => normalize(view, item)));
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ ok: false, error: 'Failed to allocate unique recordId' }, { status: 409 });
}
