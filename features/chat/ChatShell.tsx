...
export function ChatShell() {
  ...
  const [moduleRecordTarget, setModuleRecordTarget] = useState<{ view: TelecomView; recordId: string } | null>(null);
  ...
  const handleModeChange = useCallback((nextMode: AppMode) => {
    setMode(nextMode);
    setModuleRecordTarget((prev) => (
      prev && prev.view === nextMode ? prev : null
    ));
  }, []);
  ...
  const handleNavigateToRecord = useCallback((view: TelecomView, recordId: string) => {
    ...
    setContextView(view);
    setModuleRecordTarget({ view, recordId });
      ...
      <div className={cn(styles.body, !isXenaMode && styles.bodyFullWidth)}>
        {isXenaMode ? (
          ...
          <ModuleDashboard
            ...
            initialRecordId={moduleRecordTarget?.view === mode ? moduleRecordTarget.recordId : undefined}
            onBackToXena={() => handleModeChange('xena')}
          />
        )}
      </div>
    </div>
