...
export function ModuleDashboard({ view, onBackToXena, initialRecordId = null }: ModuleDashboardProps) {
  const getAuthToken = useAuthToken();
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState<StatusFilter>('all');
  const [showCreateModal, setShowCreateModal] = useState(false);
  const lastFocusedRecordRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (lastFocusedRecordRef.current === focusKey) return;
    lastFocusedRecordRef.current = focusKey;
