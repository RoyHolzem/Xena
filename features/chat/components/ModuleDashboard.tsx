...
export function ModuleDashboard({ view, onBackToXena, initialRecordId }: ModuleDashboardProps) {
  ...
  const appliedInitialRecordTarget = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!initialRecordId) {
      appliedInitialRecordTarget.current = null;
      return;
    }
    const target = `${view}:${initialRecordId}`;
    if (appliedInitialRecordTarget.current === target) return;
    appliedInitialRecordTarget.current = target;
    setSelectedRecordIds((prev) => ({ ...prev, [view]: initialRecordId }));
    void loadTelecomView(view, true, initialRecordId);
  }, [initialRecordId, loadTelecomView, setSelectedRecordIds, view]);
