...
export function ModuleDashboard({ view, onBackToXena, initialRecordId }: ModuleDashboardProps) {
  ...
  const lastInitialFocusRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (lastInitialFocusRef.current === focusKey) return;
    lastInitialFocusRef.current = focusKey;

    setSearch('');
    setStatusFilter('all');
    setSelectedRecordIds((prev) => ({ ...prev, [view]: initialRecordId }));
    void loadTelecomView(view, true, initialRecordId);
  }, [initialRecordId, view, loadTelecomView, setSelectedRecordIds]);
