...
async function createRecord(type, body) {
  ...
  const requestedRecordId = typeof body.recordId === 'string' && body.recordId.trim() ? body.recordId.trim() : '';
  ...
  return {
    statusCode: 409,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ok: false, error: 'Could not allocate a unique record ID' }),
  };
}
